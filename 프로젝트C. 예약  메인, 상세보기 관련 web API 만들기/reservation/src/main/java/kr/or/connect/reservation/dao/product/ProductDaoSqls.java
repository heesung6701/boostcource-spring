...
public class ProductDaoSqls {
  ...
  public static final String SELECT_ALL =
      "SELECT d.id, p.category_id, dii.display_info_id, c.name, p.description, p.content, p.event,"
          ...
          + " FROM display_info d" + " LEFT JOIN product p ON d.product_id = p.id"
          + " LEFT JOIN display_info_image dii ON dii.display_info_id = d.id"
          + " LEFT JOIN category c ON c.id = p.category_id" + " LIMIT :start, " + PAGE_COUNT;
  ...
  public static final String SELECT_BY_CATEGORY_ID =
      "SELECT d.id, p.category_id, dii.display_info_id, c.name, p.description, p.content, p.event,"
          ...
          + " FROM display_info d" + " LEFT JOIN product p ON d.product_id = p.id"
          + " LEFT JOIN display_info_image dii ON dii.display_info_id = d.id"
          + " LEFT JOIN category c ON c.id = p.category_id" + " WHERE p.category_id = :categoryId"
  ...
  public static final String SELECT_BY_ID =
      "SELECT d.id, p.category_id, dii.display_info_id, c.name, p.description, p.content, p.event,"
          ...
          + " FROM display_info d" + " LEFT JOIN product p ON d.product_id = p.id"
          + " LEFT JOIN display_info_image dii ON dii.display_info_id = d.id"
          + " LEFT JOIN category c ON c.id = p.category_id" + " WHERE d.id = :displayInfoId";
}
